...
/**
 * @file jabberd.cc
 * @brief Where all begins ... this file contains the main function, shutdown
...
typedef struct {
    ...
    int signalflag;  /**< set to the signal value, if jabberd receives a signal
                      */
    char *cfgfile;   /**< configuration file the user specified at the command
                        line (NULL for the default file) */
    pool cmd_line_p; /**< memory pool for command line command processing (use
                        with extrem caution, not freed before shutdown!) */
} jabberd_struct;

jabberd_struct jabberd = {NULL, NULL, 0, NULL,
                          NULL}; /**< global data for the jabberd */
...
int main(int argc, const char **argv) {
    ...
    struct poptOption options[] = {
        {"config", 'c', POPT_ARG_STRING, &(jabberd.cfgfile), 0,
         "configuration file to use", "path and filename"},
        {"include", 'i', POPT_ARG_STRING, &do_include, 0,
         "include configuration files", "comma separated list"},
        {"debugmask", 'd', POPT_ARG_INT, &do_debug, 0,
         "enable debugging (by type)", "debugging mask"},
        {"debug", 'D', POPT_ARG_NONE, NULL, 1, "enable debugging (all types)",
         NULL},
        {"zones", 'Z', POPT_ARG_STRING, &zones, 0,
         "debugging zones (file names without extension)",
         "comma separated list"},
        {"user", 'U', POPT_ARG_STRING, &run_as_user, 0,
         "run " PACKAGE " as another user", "user to run as"},
        {"home", 'H', POPT_ARG_STRING, &home, 0,
         "what to use as home directory", "directory path"},
        {"define", 'x', POPT_ARG_STRING, NULL, 2,
         "define a replacement string for configuration", "key:value"},
        {"background", 'B', POPT_ARG_NONE, &do_background, 0,
         "background the server process", NULL},
        {"host", 'h', POPT_ARG_STRING, &host, 0,
         "hostname that should be served by " PACKAGE, "domain (FQDN)"},
        {"spooldir", 's', POPT_ARG_STRING, &spool, 0,
         "directory where to place the file spool of xdb_file",
         "directory path"},
        {"import", 'I', POPT_ARG_STRING, &import_spool, 0,
         "import data to the server from a filespool", "basedir of file-spool"},
        {"version", 'V', POPT_ARG_NONE, &do_version, 0, "print server version",
         NULL},
        {NULL, 'v', POPT_ARG_NONE | POPT_ARGFLAG_DOC_HIDDEN, &do_version, 0,
    ...
    while ((pReturn = poptGetNextOpt(pCtx)) >= 0) {
        switch (pReturn) {
            ...
            case 2:
                ...
                if (c == NULL) {
                    fprintf(stderr,
                            "Invalid definition for config file replacement: "
    ...
    if (do_version != 0) {
        ...
        printf("If you need support, check out "
    ...
    /* generate a memory pool that is available for the whole livetime of
     * jabberd */
    ...
    /* the special -Z flag provides a list of zones to filter debug output for,
     * flagged w/ a simple hash */
    if (zones != NULL) {
    ...
    while (1) {
        ...
        log_debug2(ZONE, LOGT_STATUS,
                   "main load check of %.2f with %ld total threads", avload,
 ...
 *
 * The configuration file is reloaded, but not much is done with the reloaded
...
static void _jabberd_restart(jabberd_struct *j) {
    ...
    if (configurate(j->cfgfile, j->cmd_line,
                    1)) { /* failed to load.. restore the greymatter */
        log_debug2(ZONE, LOGT_CONFIG,
                   "Failed to load new config, resetting greymatter");
        log_alert(ZONE, "Failed to reload config!  Resetting internal config "
                        "-- please check your configuration!");
        greymatter__ = temp_greymatter;
        return;
    }

    /* free old greymatter (NOTE: shea, right! many internal tables/callbacs/etc
    have old config pointers :) if(temp_greymatter != NULL)
...
/**
 * do jobs, that have to be done even on fatal server shutdowns (if the exit()
 * function is called)
 *
...
static void _jabberd_atexit(void) {
    ...
    pidfile = xmlnode_get_list_item(
        xmlnode_get_tags(greymatter__, "pidfile", namespaces), 0);
