...
/**
 * @file dialback.cc
 * @brief main file of the dialback component implementing server to server
 * connections
 *
 * This is the main file of the dialback component (module) of the Jabber
 * server.
 *
 ...
 */
int dialback_check_settings(db d, mio m, const char *server, int is_outgoing,
                            int auth_type, int version) {
    int required_protection = 0;
    int protection_level = mio_is_encrypted(m);
    const char *require_tls =
        static_cast<char *>(xhash_get_by_domain(d->hosts_tls, server));
    const char *xmpp_version =
        version == -1 ? "unknown" : version == 0 ? "preXMPP" : "XMPP1.0";
    const char *auth =
        static_cast<char *>(xhash_get_by_domain(d->hosts_auth, server));
    ...
    log_debug2(ZONE, LOGT_IO,
               "requiring protection level %i for connection %s %s",
               required_protection, is_outgoing ? "to" : "from", server);
    if (protection_level < required_protection) {
        log_warn(d->i->id,
                 "stopping dialback %s %s - stream protection level (%i) of "
                 "established connection not good enough",
                 is_outgoing ? "to" : "from", server, protection_level);
        mio_write(m, NULL,
                  "<stream:error><policy-violation "
                  "xmlns='urn:ietf:params:xml:ns:xmpp-streams'/><text "
                  "xmlns='urn:ietf:params:xml:ns:xmpp-streams' "
                  "xml:lang='en'>We are configured to interconnect to your "
                  "host only using a stream protected with STARTTLS or require "
                  "a stronger encryption algorithm.</text></stream:error>",
                  -1);
        mio_close(m);
        return 0;
    }
    ...
    if (j_strcmp(auth, "db") == 0 && auth_type == 1) {
        log_warn(d->i->id,
                 "closing connection %s %s: require dialback, but SASL has "
                 "been used",
                 is_outgoing ? "to" : "from", server);
        mio_write(
            m, NULL,
            "<stream:error><policy-violation "
            "xmlns='urn:ietf:params:xml:ns:xmpp-streams'/><text "
            "xmlns='urn:ietf:params:xml:ns:xmpp-streams' xml:lang='en'>We are "
            "configured to not support SASL AUTH.</text></stream:error>",
            -1);
        mio_close(m);
        return 0;
    }
    if (j_strcmp(auth, "sasl") == 0 && auth_type == 0) {
        log_warn(d->i->id,
                 "closing connection %s %s: require SASL, but dialback has "
                 "been used",
                 is_outgoing ? "to" : "from", server);
        mio_write(
            m, NULL,
            ...
            "xmlns='urn:ietf:params:xml:ns:xmpp-streams'/><text "
            "xmlns='urn:ietf:params:xml:ns:xmpp-streams' xml:lang='en'>We are "
            "configured to not support dialback. Well, we shouldn't even have "
            "tried or advertized dialback ...</text></stream:error>",
            -1);
        mio_close(m);
        return 0;
    }
    ...
    if (protection_level < 1) {
        log_notice(d->i->id, "%s %s (unencrypted, no cert, auth=%s, stream=%s",
                   is_outgoing ? "connected to" : "connection from", server,
                   auth_type ? "sasl" : "db", xmpp_version);
        ...
        log_notice(
            d->i->id,
            "%s %s (integrity protected, %s cert is %s, auth=%s, stream=%s)",
            is_outgoing ? "connected to" : "connection from", server, certtype,
            mio_ssl_verify(m, server) ? "valid" : "invalid",
            auth_type ? "sasl" : "db", xmpp_version);
    } else {
        char sl_characteristics[1024] = "no TLS";
        if (m->ssl) {
            mio_tls_get_characteristics(m, sl_characteristics,
                                        sizeof(sl_characteristics));
        }
        ...
        log_notice(
            d->i->id,
            "%s %s (encrypted: %i b (%s), %s cert is %s, auth=%s, stream=%s)",
            is_outgoing ? "connected to" : "connection from", server,
            protection_level, m->ssl ? sl_characteristics : "no TLS", certtype,
            mio_ssl_verify(m, server) ? "valid" : "invalid",
            auth_type ? "sasl" : "db", xmpp_version);
    }
    return 1;
 ...
 */
char *dialback_merlin(pool p, char const *secret, char const *to,
                      char const *from, char const *challenge) {
    char *result = NULL;
    ...
    /* sign the message */
    hmac_sha1_ascii_r(
        secret, reinterpret_cast<unsigned char const *>(message.str().c_str()),
        message.str().length(), result);

    log_debug2(ZONE, LOGT_AUTH, "merlin casts his spell (%s - %s %s %s) %s",
               secret, to, from, challenge, result);
 ...
 *
 * @param md structure holding the mio handle and the elements to keep track of
 * idle time
...
/**
 * process a packet that has been read from a managed I/O connection and update
 * the idle time values
 *
 * @param md structure holding the elements to keep track of idle time (and
 * other elements)
...
void dialback_miod_read(miod md, xmlnode x) {
    ...
    if (jp == NULL) {
        log_warn(md->d->i->id, "dropping invalid packet from server: %s",
...
/**
 * create a new wrapper around a managed I/O connection to be able to keep track
 * about idle connections and the state of the dialback
 *
 ...
 *
 * This structure is used to pass everything that is needed to the
 * _dialback_miod_hash_cleanup callback as only one pointer can be passed to
 * this function (it is registered as a pool cleaner function) but we need
 * multiple values
 */
struct miodc {
    miod md; /**< structure holding the mio handle, idle time values and valid
                hosts */
    xht ht;  /**< hash containing outgoing connections */
    jid key; /**< key of this connection in the ht hash, format
                "destination/source" */
};

/**
 * Unregister outgoing routings, that have been routed over this connection,
 * that is closed now.
 *
...
void _dialback_miod_hash_cleanup(void *arg) {
    ...
    log_debug2(ZONE, LOGT_CLEANUP | LOGT_AUTH,
               "miod cleaning out socket %d with key %s to hash %X",
               mdc->md->m->fd, jid_full(mdc->key), mdc->ht);
    /* cool place for logging, eh? interesting way of detecting things too, *g*
     */
    if (mdc->ht == mdc->md->d->out_ok_db) {
        unregister_instance(mdc->md->d->i,
                            mdc->key->get_domain()
                                .c_str()); /* dynamic host resolution thingie */
        log_record(mdc->key->get_domain().c_str(), "out", "dialback",
                   "%d %s %s", mdc->md->count, mio_ip(mdc->md->m),
                   mdc->key->get_resource().c_str());
    } else if (mdc->ht == mdc->md->d->in_ok_db) {
        log_record(mdc->key->get_domain().c_str(), "in", "dialback", "%d %s %s",
                   mdc->md->count, mio_ip(mdc->md->m),
                   mdc->key->get_resource().c_str());
    }
}
...
void dialback_miod_hash(miod md, xht ht, jid key) {
    struct miodc *mdc;
    log_debug2(ZONE, LOGT_AUTH,
               "miod registering socket %d with key %s to hash %X", md->m->fd,
               jid_full(key), ht);
    ...
    if (ht == md->d->out_ok_db) {
        dialback_ip_set(md->d, key,
                        mio_ip(md->m)); /* save the ip since it won't be going
                                           through the dnsrv anymore */
 ...
 *
 * @param d db structure which contains the context of the dialback component
 * instance
...
char *dialback_ip_get(db d, jid host, char *ip) {
    ...
    ret =
        pstrdup(host->get_pool(),
                xmlnode_get_attrib_ns(
                    (xmlnode)xhash_get(d->nscache, host->get_domain().c_str()),
                    "i", NULL));
    log_debug2(ZONE, LOGT_IO, "returning cached ip %s for %s", ret,
               host->get_domain().c_str());
    return ret;
}
...
void dialback_ip_set(db d, jid host, char *ip) {
    ...
    xhash_put(d->nscache, xmlnode_get_attrib_ns(cache, "h", NULL),
              (void *)cache);
    log_debug2(ZONE, LOGT_IO, "cached ip %s for %s", ip,
               host->get_domain().c_str());
...
static void dialback_handle_discoinfo(db d, dpacket dp, xmlnode query, jid to) {
    ...
    requestor = jid_new(xmlnode_pool(dp->x),
                        xmlnode_get_attrib_ns(dp->x, "from", NULL));
    ...
    if (s2s_right && !to->has_node() && !to->has_resource() && node == NULL) {
        ...
        xmlnode_put_attrib_ns(x, "name", NULL, NULL,
                              "s2s component of " PACKAGE " " VERSION);
        ...
        xmlnode_put_attrib_ns(
            x, "name", NULL, NULL,
            messages_get(lang, N_("established outgoing connections")));
    } else if (s2s_right && to->get_node() == "out-connecting" &&
               !to->has_resource() && node == NULL) {
        x = xmlnode_insert_tag_ns(result, "identity", NULL, NS_DISCO_INFO);
        xmlnode_put_attrib_ns(x, "category", NULL, NULL, "hierarchy");
        xmlnode_put_attrib_ns(x, "type", NULL, NULL, "branch");
        xmlnode_put_attrib_ns(
            x, "name", NULL, NULL,
            messages_get(lang, N_("connecting outgoing connections")));
    } else if (s2s_right && to->get_node() == "in-established" &&
               !to->has_resource() && node == NULL) {
        x = xmlnode_insert_tag_ns(result, "identity", NULL, NS_DISCO_INFO);
        xmlnode_put_attrib_ns(x, "category", NULL, NULL, "hierarchy");
        xmlnode_put_attrib_ns(x, "type", NULL, NULL, "branch");
        xmlnode_put_attrib_ns(
            x, "name", NULL, NULL,
            messages_get(lang, N_("established incoming connections")));
    } else if (s2s_right && to->get_node() == "in-connecting" &&
               !to->has_resource() && node == NULL) {
        x = xmlnode_insert_tag_ns(result, "identity", NULL, NS_DISCO_INFO);
        xmlnode_put_attrib_ns(x, "category", NULL, NULL, "hierarchy");
        xmlnode_put_attrib_ns(x, "type", NULL, NULL, "branch");
        xmlnode_put_attrib_ns(
            x, "name", NULL, NULL,
            messages_get(lang, N_("connecting incoming connections")));
    } else if (s2s_right && to->get_node() == "out-established" &&
               to->has_resource() && node == NULL) {
        x = xmlnode_insert_tag_ns(result, "identity", NULL, NS_DISCO_INFO);
        xmlnode_put_attrib_ns(x, "category", NULL, NULL, "hierarchy");
        xmlnode_put_attrib_ns(x, "type", NULL, NULL, "branch");
        xmlnode_put_attrib_ns(x, "name", NULL, NULL,
                              to->get_resource().c_str());
    } else if (s2s_right && to->get_node() == "out-established" &&
               to->has_resource() && j_strcmp(node, "last") == 0) {
        miod md = static_cast<miod>(
            xhash_get(d->out_ok_db, to->get_resource().c_str()));
        ...
        if (md == NULL) {
            ...
            xmlnode_put_attrib_ns(
                x, "name", NULL, NULL,
                messages_get(lang, N_("-- connection gone --")));
        } else {
            struct tm last_utc;
            time_t last = md->last;
            ...
            if (gmtime_r(&last, &last_utc)) {
               char last_str[36+100];                  //Shahid: added +100 because of compilation error of out of buffer
                ...
                snprintf(last_str, sizeof(last_str),
                         ...
                         messages_get(lang, N_("Last used:")),
                         1900 + last_utc.tm_year, last_utc.tm_mon + 1,
                         last_utc.tm_mday, last_utc.tm_hour, last_utc.tm_min,
                         last_utc.tm_sec);

                x = xmlnode_insert_tag_ns(result, "identity", NULL,
                                          NS_DISCO_INFO);
                xmlnode_put_attrib_ns(x, "category", NULL, NULL, "hierarchy");
                xmlnode_put_attrib_ns(x, "type", NULL, NULL, "leaf");
               ...
               to->has_resource() && j_strcmp(node, "count") == 0) {
        miod md = static_cast<miod>(
            xhash_get(d->out_ok_db, to->get_resource().c_str()));
        ...
        if (md == NULL) {
            ...
            snprintf(name_str, sizeof(name_str), "%s %d",
                     messages_get(lang, N_("Stanza count:")), md->count);
            ...
            xmlnode_put_attrib_ns(x, "name", NULL, NULL, name_str);
               ...
               to->has_resource() && j_strcmp(node, "ip") == 0) {
        miod md = static_cast<miod>(
            xhash_get(d->out_ok_db, to->get_resource().c_str()));
        ...
        if (md == NULL || md->m == NULL) {
            ...
            snprintf(ip_str, sizeof(ip_str), "%s %s=%s;%d %s=%s;%d %s=%s",
                     ...
                     messages_get(lang, N_("local")), md->m->our_ip,
                     md->m->our_port, messages_get(lang, N_("remote")),
                     md->m->peer_ip, md->m->peer_port,
                     messages_get(lang, N_("tls")),
                     md->m->ssl == NULL ? messages_get(lang, N_("no"))
                                        : messages_get(lang, N_("yes")));
        ...
        xmlnode_put_attrib_ns(x, "name", NULL, NULL,
               ...
               to->has_resource() && node != NULL) {
        dboc dc = static_cast<dboc>(
            xhash_get(d->out_connecting, to->get_resource().c_str()));
        ...
        if (dc == NULL) {
            ...
            xmlnode_put_attrib_ns(
                x, "name", NULL, NULL,
                messages_get(lang,
                             N_("-- connection gone or now established --")));
            ...
            if (dc->m != NULL) {
                snprintf(ip_str, sizeof(ip_str), "%s: %s=%s;%d %s=%s;%d %s=%s",
                         messages_get(lang, N_("IP/port:")),
                         messages_get(lang, N_("local")), dc->m->our_ip,
                         dc->m->our_port, messages_get(lang, N_("remote")),
                         dc->m->peer_ip, dc->m->peer_port,
                         messages_get(lang, N_("tls")),
                         dc->m->ssl == NULL ? messages_get(lang, N_("no"))
                                            : messages_get(lang, N_("yes")));
            } else {
                snprintf(
                    ip_str, sizeof(ip_str), "%s",
                    messages_get(lang, N_("IP/port: no current connection")));
            }
            ...
            if (gmtime_r(&last, &last_utc)) {
                ...
                snprintf(last_str, sizeof(last_str),
                         "%s %d-%02d-%02d %02d:%02d:%02d UTC",
                         messages_get(lang, N_("Connection start:")),
                         1900 + last_utc.tm_year, last_utc.tm_mon + 1,
                         last_utc.tm_mday, last_utc.tm_hour, last_utc.tm_min,
                         last_utc.tm_sec);
            ...
            xmlnode_put_attrib_ns(
                x, "name", NULL, NULL,
                dc->xmpp_version < 0
                    ? messages_get(lang, N_("XMPP version: unknown"))
                    : dc->xmpp_version
                          ? messages_get(lang, N_("XMPP version: 1.0"))
                          : messages_get(lang, N_("XMPP version: 0.9")));
        } else if (j_strcmp(node, "pendingip") == 0) {
            x = xmlnode_insert_tag_ns(result, "identity", NULL, NS_DISCO_INFO);
            xmlnode_put_attrib_ns(x, "category", NULL, NULL, "hierarchy");
            xmlnode_put_attrib_ns(x, "type", NULL, NULL, "leaf");
            std::ostringstream name;
            name << messages_get(lang, N_("Pending IPs: "))
                 << (dc->ip ? dc->ip
                            : messages_get(
                                  lang, N_("no more other IPs will be tried")));
            ...
            switch (dc->db_state) {
                case not_requested:
                    name << messages_get(
                        lang, N_("no dialback request, just sending verifies"));
                    break;
                case could_request:
                    name << messages_get(
                        lang,
                        N_("we could send dialback requests, if we want to"));
                    break;
                case want_request:
                    name << messages_get(
                        lang, N_("we want to send dialback requests, "
                                 "but cannot do that yet"));
                    break;
                case sent_request:
                    name << messages_get(lang,
                                         N_("we have sent a dialback request"));
                    break;
                default:
            ...
            switch (dc->connection_state) {
                case created:
                    name << messages_get(
                        lang, N_("created, not yet started to connect"));
                    break;
                case connecting:
                    name << messages_get(
                        lang, N_("we started to connect, no connection yet"));
                    break;
                case connected:
                    name << messages_get(lang,
                                         N_("connected to the other host"));
                    break;
                case got_streamroot:
                ...
                case waiting_features:
                    name << messages_get(
                        lang, N_("we are waiting for stream features"));
                    break;
                case got_features:
                    name << messages_get(lang,
                                         N_("we got the stream features"));
                    break;
                case sent_db_request:
                    name << messages_get(lang,
                                         N_("we sent out a dialback request"));
                    break;
                case db_succeeded:
                    name << messages_get(
                        lang, N_("we had success with our dialback request"));
                    break;
                case db_failed:
                ...
                case sasl_started:
                    name << messages_get(
                        lang, N_("we started to authenticate using sasl"));
                    break;
                case sasl_fail:
                    name << messages_get(
                        lang, N_("there was a SASL authentication failure"));
                    break;
                case sasl_success:
                    name << messages_get(
                        lang, N_("successfully authenticated using SASL"));
                    break;
                default:
                    name << messages_get(lang, N_("invalid"));
            }
            ...
            xmlnode_put_attrib_ns(x, "type", NULL, NULL, "leaf");
            std::ostringstream name;
            name << messages_get(lang, N_("Dialback: "))
                 << (dc->flags.db
                         ? messages_get(lang, N_("supported by peer"))
                         : messages_get(lang, N_("unsupported by peer")));
            xmlnode_put_attrib_ns(x, "name", NULL, NULL, name.str().c_str());
            ...
            xmlnode_put_attrib_ns(x, "type", NULL, NULL, "leaf");
            std::ostringstream name;
            name << messages_get(lang, N_("Dropped because of settings: "))
                 << (dc->settings_failed ? messages_get(lang, N_("yes"))
                                         : messages_get(lang, N_("no")));
            xmlnode_put_attrib_ns(x, "name", NULL, NULL, name.str().c_str());
        } else if (j_strcmp(node, "verifies") == 0) {
            int count = 0;
            xmlnode iter = NULL;

            for (iter = xmlnode_get_firstchild(dc->verifies); iter != NULL;
                 iter = xmlnode_get_nextsibling(iter)) {
                count++;
            }
        ...
        xmlnode_put_attrib_ns(x, "name", NULL, NULL,
               ...
               to->has_resource() && j_strcmp(node, "last") == 0) {
        miod md = static_cast<miod>(
            xhash_get(d->in_ok_db, to->get_resource().c_str()));
        ...
        xmlnode_put_attrib_ns(x, "name", NULL, NULL,
               ...
               to->has_resource() && node != NULL) {
        dbic c =
            static_cast<dbic>(xhash_get(d->in_id, to->get_resource().c_str()));
        ...
        if (c == NULL) {
            ...
            if (c->m != NULL) {
                snprintf(ip_str, sizeof(ip_str), "%s %s=%s;%d %s=%s;%d %s=%s",
                         messages_get(lang, N_("IP/port:")),
                         messages_get(lang, N_("local")), c->m->our_ip,
                         c->m->our_port, messages_get(lang, N_("remote")),
                         c->m->peer_ip, c->m->peer_port,
                         messages_get(lang, N_("tls")),
                         c->m->ssl == NULL ? messages_get(lang, N_("no"))
                                           : messages_get(lang, N_("yes")));
            } else {
                snprintf(
                    ip_str, sizeof(ip_str), "%s",
            ...
            xmlnode_put_attrib_ns(x, "type", NULL, NULL, "leaf");
            std::ostringstream name;
            name << messages_get(lang, N_("Addresses:")) << " "
                 << messages_get(lang, N_("local")) << "=" << c->we_domain
                 << ", " << messages_get(lang, N_("peer")) << "="
                 << c->other_domain;
            xmlnode_put_attrib_ns(x, "name", NULL, NULL, name.str().c_str());
        } else if (j_strcmp(node, "xmppversion") == 0) {
            x = xmlnode_insert_tag_ns(result, "identity", NULL, NS_DISCO_INFO);
            ...
            switch (c->xmpp_version) {
                ...
                case 0:
                    ...
                    break;
                default:
                    name << messages_get(lang, N_("XMPP version: unknown"))
                         << " (" << c->xmpp_version << ")";
            }
            xmlnode_put_attrib_ns(x, "name", NULL, NULL, name.str().c_str());
        } else if (j_strcmp(node, "last") == 0) {
...
/**
 * iterate the xhash of established outgoing connections and add items for them
 * to a disco#items query
 */
void _dialback_walk_out_established(xht h, const char *key, void *value,
                                    void *arg) {
...
/**
 * iterate the xhash of connecting outgoing connections and add items for them
 * to a disco#items query
 */
void _dialback_walk_out_connecting(xht h, const char *key, void *value,
                                   void *arg) {
    _dialback_jid_with_xmlnode *jx = (_dialback_jid_with_xmlnode *)arg;
    xmlnode item = NULL;
...
/**
 * iterate the xhash of established incomming connections and add items for them
 * to a disco#items query
 */
void _dialback_walk_in_established(xht h, const char *key, void *value,
                                   void *arg) {
    _dialback_jid_with_xmlnode *jx = (_dialback_jid_with_xmlnode *)arg;
    xmlnode item = NULL;
...
/**
 * iterate the xhash of connecting incomming connections and add items for them
 * to a disco#items query
 */
void _dialback_walk_in_connecting(xht h, const char *key, void *value,
                                  void *arg) {
    _dialback_jid_with_xmlnode *jx = (_dialback_jid_with_xmlnode *)arg;
    xmlnode item = NULL;
 ...
 */
static void dialback_handle_discoitems(db d, dpacket dp, xmlnode query,
                                       jid to) {
    const char *node = xmlnode_get_attrib_ns(query, "node", NULL);
    xmlnode result = NULL;
    xmlnode x = NULL;
    ...
    if (!to->has_node() && !to->has_resource() && node == NULL && s2s_right) {
        ...
        xhash_walk(d->out_ok_db, _dialback_walk_out_established, (void *)&jx);
               ...
               to->has_resource() && node == NULL) {
        jid other_server =
            jid_new(xmlnode_pool(result), to->get_resource().c_str());
        ...
        x = xmlnode_insert_tag_ns(result, "item", NULL, NS_DISCO_ITEMS);
        xmlnode_put_attrib_ns(x, "name", NULL, NULL,
                              messages_get(lang, N_("Stanza count")));
        xmlnode_put_attrib_ns(x, "jid", NULL, NULL, jid_full(to));
        ...
        if (other_server != NULL) {
            x = xmlnode_insert_tag_ns(result, "item", NULL, NS_DISCO_ITEMS);
            xmlnode_put_attrib_ns(x, "name", NULL, NULL,
                                  other_server->get_domain().c_str());
            xmlnode_put_attrib_ns(x, "jid", NULL, NULL,
                                  other_server->get_domain().c_str());
        }
    } else if (s2s_right && to->get_node() == "out-connecting" &&
               !to->has_resource() && node == NULL) {
        ...
        xhash_walk(d->out_connecting, _dialback_walk_out_connecting,
                   (void *)&jx);
    } else if (s2s_right && to->get_node() == "out-connecting" &&
               to->has_resource() && node == NULL) {
        x = xmlnode_insert_tag_ns(result, "item", NULL, NS_DISCO_ITEMS);
        ...
        if (other_server != NULL) {
            x = xmlnode_insert_tag_ns(result, "item", NULL, NS_DISCO_ITEMS);
            xmlnode_put_attrib_ns(x, "name", NULL, NULL,
                                  other_server->get_resource().c_str());
            xmlnode_put_attrib_ns(x, "jid", NULL, NULL,
                                  other_server->get_resource().c_str());
        }
    } else if (s2s_right && to->get_node() == "in-connecting" &&
        ...
        x = xmlnode_insert_tag_ns(result, "item", NULL, NS_DISCO_ITEMS);
        xmlnode_put_attrib_ns(x, "name", NULL, NULL,
                              messages_get(lang, N_("Addresses")));
        xmlnode_put_attrib_ns(x, "jid", NULL, NULL, jid_full(to));
 ...
 *
 * This is where the dialback instance receives packets from the jabberd
 * framework
 *
 * @param i the dialback instance we are running in
 * @param dp the dialback packet
 * @param arg pointer to the db structure with the context of the dialback
 * component instance
...
result dialback_packets(instance i, dpacket dp, void *arg) {
    ...
    if (j_strcmp(xmlnode_get_name(x), "verify") == 0 &&
        j_strcmp(xmlnode_get_namespace(x), NS_DIALBACK) == 0) {
        xmlnode_put_attrib_ns(x, "to", NULL, NULL,
                              xmlnode_get_attrib_ns(x, "ofrom", NULL));
    ...
    if (d->i->id && to->get_domain() == d->i->id) {
        ...
        x2 = xmlnode_get_list_item(
            xmlnode_get_tags(x, "discoinfo:query", d->std_ns_prefixes), 0);
        if (x2 != NULL) {
            dialback_handle_discoinfo(d, dp, x2, to);
            return r_DONE;
        }
        x2 = xmlnode_get_list_item(
            xmlnode_get_tags(x, "discoitems:query", d->std_ns_prefixes), 0);
        if (x2 != NULL) {
...
/**
 * callback for walking each miod-value host hash tree, close connections that
 * have been idle to long
 *
 * The timeout value is configured in the dialback component configuration using
 * the &lt;idletimeout/&gt; element.
 *
 * @param h the hash table containing all connections
 * @param key unused/ignored (the key of the value in the hash table)
 * @param data the value in the hash table = the structure holding the
 * connection
...
void _dialback_beat_idle(xht h, const char *key, void *data, void *arg) {
    ...
    if (((int)*(time_t *)arg - md->last) >= md->d->timeout_idle) {
        log_debug2(ZONE, LOGT_IO, "Idle Timeout on socket %d to %s", md->m->fd,
                   mio_ip(md->m));
...
/**
 * callback for walking incoming connections, that are not authorized yet,
 * checking for timeotus
 *
 * @param h the hash table containing all connections
 * @param key unused/ignored (the key of the value in the hash table)
 * @param data the value in the hash table = the structure holding the
...
void _dialback_beat_in_idle(xht h, const char *key, void *data, void *arg) {
    ...
    if (((int)*(time_t *)arg - c->stamp) >= c->d->timeout_auth) {
        log_debug2(ZONE, LOGT_IO, "Idle Timeout on socket %d to %s", c->m->fd,
                   mio_ip(c->m));
...
/**
 * callback for walking outgoing connections, that are not authorized yet,
 * checking for timeotus
 *
 * @param h the hash table containing all connections
 * @param key unused/ignored (the key of the value in the hash table)
...
/**
 * initiate walking the hash of existing s2s connections to check if they have
 * been idle to long
 *
 * called as a heartbeat function
 *
 * @param arg pointer to the structure holding the context of the dialback
 * component instance
 * @return always r_DONE
 */
...
/**
 * we pass a token in the stream root to identify a looping connection to
 * ourself. This generated the token of the server.
 *
 * @param d the dialback instance
...
extern "C" void dialback(instance i, xmlnode x) {
    ...
    /* get the config */
    d->xc = xdb_cache(i);
    cfg = xdb_get(d->xc, jid_new(xmlnode_pool(x), "config@-internal"),
                  NS_JABBERD_CONFIG_DIALBACK);
    ...
    xhash_put(d->std_ns_prefixes, "wrap",
    ...
    xhash_put(d->std_ns_prefixes, "conf",
              const_cast<char *>(NS_JABBERD_CONFIG_DIALBACK));
    xhash_put(d->std_ns_prefixes, "discoinfo",
              const_cast<char *>(NS_DISCO_INFO));
    xhash_put(d->std_ns_prefixes, "discoitems",
              const_cast<char *>(NS_DISCO_ITEMS));

    max = j_atoi(
        xmlnode_get_list_item_data(
            xmlnode_get_tags(cfg, "conf:maxhosts", d->std_ns_prefixes), 0),
        997);
    ...
    d->i = i;
    d->timeout_idle = j_atoi(
        xmlnode_get_list_item_data(
            xmlnode_get_tags(cfg, "conf:idletimeout", d->std_ns_prefixes), 0),
        900);
    d->timeout_packets = j_atoi(
        xmlnode_get_list_item_data(
            xmlnode_get_tags(cfg, "conf:queuetimeout", d->std_ns_prefixes), 0),
        30);
    d->timeout_auth = j_atoi(
        xmlnode_get_list_item_data(
            xmlnode_get_tags(cfg, "conf:authtimeout", d->std_ns_prefixes), 0),
        d->timeout_idle);
    d->secret = pstrdup(
        i->p, xmlnode_get_list_item_data(
                  xmlnode_get_tags(cfg, "conf:secret", d->std_ns_prefixes), 0));
    if (d->secret ==
        NULL) /* if there's no configured secret, make one on the fly */
        d->secret = pstrdup(i->p, dialback_randstr());

    /* Get rate info if it exists */
    cur = xmlnode_get_list_item(
        xmlnode_get_tags(cfg, "conf:rate", d->std_ns_prefixes), 0);
    if (cur != NULL) {
    ...
    cur = xmlnode_get_list_item(
        xmlnode_get_tags(cfg, "conf:karma", d->std_ns_prefixes), 0);
    if (cur != NULL) {
        k.val = j_atoi(
            xmlnode_get_list_item_data(
                xmlnode_get_tags(cur, "conf:init", d->std_ns_prefixes), 0),
            KARMA_INIT);
        k.max = j_atoi(
            xmlnode_get_list_item_data(
                xmlnode_get_tags(cur, "conf:max", d->std_ns_prefixes), 0),
            KARMA_MAX);
        k.inc = j_atoi(
            xmlnode_get_list_item_data(
                xmlnode_get_tags(cur, "conf:inc", d->std_ns_prefixes), 0),
            KARMA_INC);
        k.dec = j_atoi(
            xmlnode_get_list_item_data(
                xmlnode_get_tags(cur, "conf:dec", d->std_ns_prefixes), 0),
            KARMA_DEC);
        k.restore = j_atoi(
            xmlnode_get_list_item_data(
                xmlnode_get_tags(cur, "conf:restore", d->std_ns_prefixes), 0),
            KARMA_RESTORE);
        k.penalty = j_atoi(
            xmlnode_get_list_item_data(
                xmlnode_get_tags(cur, "conf:penalty", d->std_ns_prefixes), 0),
            KARMA_PENALTY);
        k.reset_meter = j_atoi(
            xmlnode_get_list_item_data(
                xmlnode_get_tags(cur, "conf:resetmeter", d->std_ns_prefixes),
                0),
            KARMA_RESETMETER);
    ...
    if (ips.size() > 0) {
        ...
        for (iter = ips.begin(); iter != ips.end(); ++iter) {
            mio m;
            m = mio_listen(
                j_atoi(xmlnode_get_attrib_ns(*iter, "port", NULL), 5269),
                xmlnode_get_data(*iter), dialback_in_read, (void *)d,
                MIO_LISTEN_XML);
            if (m == NULL)
                return;
    ...
    xmlnode_vector hosts =
        xmlnode_get_tags(cfg, "conf:host", d->std_ns_prefixes);
    ...
    for (cur_item = hosts.begin(); cur_item != hosts.end(); ++cur_item) {
        ...
        char *auth = NULL;
        char *hostname =
            pstrdup(i->p, xmlnode_get_attrib_ns(*cur_item, "name", NULL));
    ...
    register_beat(d->timeout_idle < 60 || d->timeout_auth < 60
                      ? (d->timeout_idle < d->timeout_auth ? d->timeout_idle
                                                           : d->timeout_auth)
                      : 60,
                  dialback_beat_idle, (void *)d);
